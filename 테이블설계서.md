...
| MEM_EMAIL        | VARCHAR2(30)  | 회원이메일, 필수항목                                   | NOT NULL, UNIQUE |
| MEM_PASSWORD     | VARCHAR2(30)  | 회원비밀번호                                           |     NOT NULL     |
| REFRESH_TOKEN    | VARCHAR2(30)  | 리프레쉬 토큰                                          |      UNIQUE      |
| PLATFORM         | VARCHAR2(30)  | DEFAULT 'NORMAL'. 'KAKAO', 'GOOGLE', 'NAVER' 중 1      |      CHECK       |
...
| MEM_NO        |   NUMBER   | 회원번호, 시퀀스 사용      |        FOREIGN KEY (MEMBER)         |
| MEM_EXP_VALUE |   NUMBER   | DEFAULT 0, 획득 경험치     |              NOT NULL               |
| MEM_EXP_DATE  |    DATE    | DEFAULT SYSDATE, 획득 날짜 |              NOT NULL               |
| RES_CODE      |   NUMBER   | 예약코드                   | FOREIGN KEY (RESERVATION), NOT NULL |
...
| ACC_NO                 |    NUMBER     | 숙박업소번호            | FOREIGN KEY (ACCOMMODATION), NOT NULL |
...
| COUPON_NO          |   NUMBER   | 쿠폰번호                        | FOREIGN KEY (COUPON) |
| MEM_NO             |   NUMBER   | 회원번호                        | FOREIGN KEY (MEMBER) |
...
| MEM_ID          | VARCHAR2(30) | 회원번호                        |   FOREIGN KEY (MEMBER), NOT NULL    |
...
| ACC_NO         |    NUMBER    | 숙박업소번호                           | FOREIGN KEY (ACCOMMODATION), NOT NULL |
| ROOM_NO        |    NUMBER    | 객실번호                               |     FOREIGN KEY (ROOM), NOT NULL      |
| MEM_NO         |    NUMBER    | 회원번호                               |    FOREIGN KEY (MEMBER), NOT NULL     |
| MEM_EMAIL      |    NUMBER    | 회원이메일                             |           UNIQUE, NOT NULL            |
...
| RES_STATUS     | VARCHAR2(20) | 예약 상태 (예약완료, 처리중, 예약취소) |                 CHECK                 |
| PAY_NO         |    NUMBER    | 결제번호                               |         FOREIGN KEY (PAY_NO)          |
...
| MEM_NO                |    NUMBER     | 회원번호                |    FOREIGN KEY (MEMBER), NOT NULL     |
| ACC_NO                |    NUMBER     | 숙박업소번호            | FOREIGN KEY (ACCOMMODATION), NOT NULL |
| ROOM_NO               |    NUMBER     | 객실번호(객실호수X)     |     FOREIGN KEY (ROOM), NOT NULL      |
...
| REV_STATUS            | VARCHAR2(10)  | 'PUBLIC' OR 'PRIVATE'   |            CHECK, NOT NULL            |
| REGISTERED_DATE       |     DATE      | DEFAULT SYSDATE, 등록일 |               NOT NULL                |
...
| MEM_NO          | VARCHAR2(30)  | 회원번호                | FOREIGN KEY (MEMBER), NOT NULL |
| REGISTERED_DATE |     DATE      | DEFAULT SYSDATE, 등록일 |            NOT NULL            |
...
| CHAT_CATEGORY   | VARCHAR2(20) | 문의 카테고리. '기타문의', '예약문의' 중 1 |             CHECK, NOT NULL              |
| REGISTERED_DATE |     DATE     | DEFAULT SYSDATE, 등록일                    |                 NOT NULL                 |
| MEM_EMAIL       |    NUMBER    | 채팅 대상 이메일                           | FOREIGN KEY MEMBER (MEM_EMAIL), NOT NULL |
| ADMIN_NO        |    NUMBER    | 담당 관리자 번호                           |      FOREIGN KEY (ADMIN), NOT NULL       |
...
| CHAT_ROOM_NO    |    NUMBER     | 채팅방번호                 |    FOREIGN KEY (CHAT_ROOM), NOT NULL     |
| SENDER_EMAIL    |    NUMBER     | 메시지 보낸 사용자 이메일  | FOREIGN KEY MEMBER (MEM_EMAIL), NOT NULL |
